...
func generateFileContent(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile) {
	...
	for _, enum := range file.Enums {
		skipReason := genErrorsReason(gen, file, g, enum)
		if !skipReason {
			index++
		}
	}
...
func genErrorsReason(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, enum *protogen.Enum) bool {
	...
	defaultStat := proto.GetExtension(enum.Desc.Options(), errors.E_DefaultStat)
	stat := 0
	if ok := defaultStat.(int32); ok != 0 {
		stat = int(ok)
	}
	defaultMsg := proto.GetExtension(enum.Desc.Options(), errors.E_DefaultMsg)
	msg := ""
	if strOk := defaultMsg.(string); strOk != "" {
		msg = strOk
	}
	...
	for _, v := range enum.Values {
		...
		eStat := proto.GetExtension(v.Desc.Options(), errors.E_Stat)
		if ok := eStat.(int32); ok != 0 {
			enumStat = int(ok)
		}
		eMsg := proto.GetExtension(v.Desc.Options(), errors.E_Msg)
		if strOk := eMsg.(string); strOk != "" {
			enumMsg = strOk
		}
